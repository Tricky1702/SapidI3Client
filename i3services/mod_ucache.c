...
staticv object  i3;
staticv string array local_ucacheUpdated;
staticv string array shadows;
...
varargs void addUserCache(string mudname, string username, string visname, int gender, int newuser)
{
    ...
    else username = TERMINAL_D->parse_pinkfish(username, "dumb");
...
varargs void removeUserCache(string mudname, string username)
{
    ...
    if (!undefinedp(username))
        username = TERMINAL_D->parse_pinkfish(username, "dumb");

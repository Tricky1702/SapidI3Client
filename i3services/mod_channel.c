...
staticv object       I3, ChanD, HistD;
staticv mapping      ChanIndex, AllowList;
staticv string array Listening;

mapping ChanList;

staticf int load_allow_list();
...
void remove()
{
    ...
    {
        string real_channel = ChanD->query_real_channel_name(channel);

        send_channel_listen(real_channel, 0);
        ChanD->removeCh(channel);
        HistD->rem_history(real_channel);
        HistD->rem_file_history(real_channel);
    }
...
{
    string array lines;
    string file = read_file(I3_ALLOW_LIST);
...
void chanlist_handler(mixed array packet)
{
    ...
    if (I3->getData("chanlist_id") == chanlist_id)
        return;
    ...
    I3->setData("chanlist_id", chanlist_id);
    ANNOUNCE_CHDMOD->rec_m_msg(
      "intermud3",
      "I3_CHANNEL",
...
void channel_m_handler(mixed array packet)
{
    ...
    channel = packet[6];
    tmp = message = convert_from_utf8(packet[8]);
